...
export default function Home() {
  ...
  async function refresh() { const next = await request('state'); setData(next); if (next.config) { setModel(next.config.model); setMode(next.config.mode); } return next as Snapshot; }
  ...
  async function processBatch(single: boolean, regenerate = false) {
    ...
    await action(async () => {
      ...
      try {
        await request('instruction', { instruction });
        ...
        const result = await runConcurrent(single ? queue.slice(0, 1) : queue, {
          ...
          run: async job => {
            ...
            try {
              const updated = await request<Job>('edit', { id: job.id, key, instruction, ...(regenerate ? {regenerate:true,expectedUpdated:job.updated} : {}) });
  ...
  return <div className="studio">
    ...
    <main className="workspace">
      ...
      <div className="work-grid"><section className="source-area">
        ...
        <label className="field-label" htmlFor="instruction">Что заменить</label><Textarea id="instruction" value={instruction} maxLength={3000} disabled={busy || !ready} onChange={e => setInstruction(e.target.value)} rows={5} />{data.jobs.length > 0 && <><p className="help">Правьте инструкцию в этой сессии: новый текст применяется при следующем запуске, в том числе к выбранным для перегенерации фото. Опишите желаемый результат целиком: модель получает исходное фото, а не предыдущую генерацию.</p><Button variant="outline" disabled={busy || !instruction.trim() || instruction === data.config?.instruction} onClick={() => action(async () => { await request('instruction', {instruction}); setMessage('Промпт сохранён. Он будет использован при следующем запуске.'); })}>Сохранить промпт</Button></>}<p className="help">Один новый макет на фотографию. Все копии одного макета имеют одинаковый физический размер и пропорции; учитываются перспектива и изгиб поверхности.</p>
